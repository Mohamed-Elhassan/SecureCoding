...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    // Create a lower-case copy for case-insensitive searching
    std::string lower_sql = sql;
    std::transform(lower_sql.begin(), lower_sql.end(), lower_sql.begin(), ::tolower);

    // Check for common SQL injection patterns
    // Only flag injection attempts if the query contains known injection patterns
    ...
    for (const auto& pattern : injection_patterns)
    {
        ...
        {
            std::cout << "Possible SQL Injection detected in query: " << sql << std::endl;
            return false;
        }
    }
